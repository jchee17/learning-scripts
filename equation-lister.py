...
def findEqnSent(input, eqns):
    ...
    # \label{eq:arem_formula}\ 
    labelPat   = re.compile(r"(\\label{eq:)(.*?)(})")
    refPat     = re.compile(r"(\\ref{eq:|\\eqref{eq:)(.*?)(})")
    ...
    while len(unread)>0:
        ...
        if multilineFlag:
            continue
        #temp.append(current)
        temp = temp + " " + current.rstrip()
    unread = temp

    sents = tokenize.sent_tokenize(unread)
    labels = []

    outputData = {}
    for s in sents:
      s = s.replace("~", " ")
      matchRef = refPat.search(s)
      if matchRef:
        refLabel = matchRef.group(2) 
        #TODO: this if statement is required because multiple labels in one eqnarray is not currently supported
        if refLabel in eqns:
          if refLabel in outputData:
            (a,sentList) = outputData[refLabel]
            sentList.append(s)
            outputData[refLabel] = (a,sentList)
          else: 
            outputData[refLabel] = (eqns[refLabel], [s])
    
    for label in outputData:
      (eqn, sent) = outputData[label]
      print "LABEL: {}, EQUATION: {}, SENTENCES: {}".format(label, eqn, sent)
      
def getEqnDict(input, output='out'):
    inputHandler = open(input, 'r')
    unread = inputHandler.readlines()
    inputHandler.close()
    multilineFlag = False
    ...
    while len(unread)>0:
        ...
        if matchFormula:
            ...
            matchLabel = labelPat.search(''.join(formula_temp))
            if matchLabel:
              eqnDict[matchLabel.group(2)] = ''.join(formula_temp)
...
def main():
    ...
    if args.directory:
        ...
        eqns   = getEqnDict(args.inputName)
        findEqnSent(args.inputName,eqns)
